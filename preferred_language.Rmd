...
# STEP 4: Identify earliest RASS time for each hospitalization
...
# Form new table with earliest RASS times
hosp_first_rass <- hosp_time_rass_filt[, .(first_rass_dttm = min(recorded_dttm)), by = hospitalization_id]

# Merge first RASS time with Hosp Time Clean to form Current Timeline
hosp_rass_timeline <- merge(hosp_time_clean, hosp_first_rass, by = "hospitalization_id", all.x = TRUE)


# STEP 5: Calculate time between IMV and first RASS and flag those with greater than 8 hours between
...
# STEP 6: Remove All Hospitalizations with Delayed RASS Times
hosp_rass_clean <- hosp_rass_timeline[flag_greater_than_8h == FALSE | is.na(flag_greater_than_8h) | flag_greater_than_8h == ""]
hosp_rass_clean[, c("first_rass_dttm", "time_to_first_rass", "flag_greater_than_8h") := NULL]
...
paralytic_clean <- merge(hosp_rass_clean, earliest_paralytics, by = "hospitalization_id", all.x = TRUE)
