...
packages <- c("zoo", "data.table", "dplyr", "lubridate", "arrow")
...
# PART SEVEN: Clean any patients receiving continuous paralytics in the first 72 hours of admission

```{R}

meds <- fread(paste0(data_dir,"medication_admin_continuous.csv.gz"))[hospitalization_id %in% hosp_time_clean_id]

# Filter only for paralytic meds
meds_paralytics <- meds[med_group == "paralytics"]

# Filter only for doses >0
meds_paralytics_filtered <- meds_paralytics[med_dose > 0]

# Add a new column with the earliest admin_dttm where med_dose > 0 for each hospitalization_id
earliest_paralytics <- meds_paralytics[, .(earliest_paralytic_dttm = min(admin_dttm)), by = hospitalization_id]

# Merge with hosp_time_clean
paralytic_clean <- merge(hosp_time_clean, earliest_paralytics, by = "hospitalization_id", all.x = TRUE)

# Identify patients paralyzed within first 72 hours after beginning mechanical ventilation
paralytic_clean[, paralyzed := as.logical(abs(difftime(earliest_paralytic_dttm, begin_imv, units = "hours")) <= 72)]

num_paralyzed <- paralytic_clean[paralyzed == TRUE, .N]

# Remove All Hospitalizations with Paralytics in First 72 hours
hosp_paralytic_clean <- paralytic_clean[paralyzed == FALSE | is.na(paralyzed) | paralyzed == ""]

hosp_paralytic_clean_id <- unique(hosp_paralytic_clean$hospitalization_id)


```


# PART EIGHT: Final Cleaning Step - Rule Out Missing RASS Values Patients

```{R}

# Load Patient Assessments For Hospitalization_IDs above
patient_assessments <- fread("/share/projects/data/circe/v20240331/clif/patient_assessments.csv.gz", 
                             select = c("hospitalization_id", "recorded_dttm", "assessment_category", 
                                        "assessment_group", "numerical_value", "categorical_value", "text_value"))[hospitalization_id %in% hosp_paralytic_clean_id]
...
hosp_time_rass <- merge(hosp_time_clean, pat_assess_rass, by = "hospitalization_id", all.x = TRUE)

# Subtract 2 hours from begin_imv and apply the filters
hosp_time_rass_filt <- hosp_time_rass[recorded_dttm > (begin_imv - hours(2)) & recorded_dttm < end_imv]

# Identify Earliest Documented RASS Score for Each hospitalization_id
hosp_first_rass <- hosp_time_rass_filt[, .(first_rass_dttm = min(recorded_dttm)), by = hospitalization_id]

# Merge
hosp_rass_timeline <- merge(hosp_paralytic_clean, hosp_first_rass, by = "hospitalization_id", all.x = TRUE)


# Calculate Time Between First RASS and Begin IMV
hosp_rass_timeline[, time_to_first_rass := as.numeric(difftime(first_rass_dttm, begin_imv, units = "hours"))]

# Flag Those with 8 hour gaps 
hosp_rass_timeline[, flag_greater_than_8h := time_to_first_rass > 8]

num_rass_missing <- hosp_rass_timeline[flag_greater_than_8h == TRUE, .N]

# Remove All Hospitalizations with Delayed RASS Times
hosp_rass_clean <- hosp_rass_timeline[flag_greater_than_8h == FALSE | is.na(flag_greater_than_8h) | flag_greater_than_8h == ""]


hosp_rass_clean %>% distinct(patient_id)


hosp_id_clean <- unique(hosp_rass_clean$hospitalization_id)
pat_id_clean <- unique(hosp_rass_clean$patient_id)
...
# 6 Hours before IMV until 72 hours after begin IMV - this gives us access to lab data that may affect the decision to intubate the patient. We will only begin calculation of SOFA Score, etc once IMV begins but we will carry forward values of labs, etc from before that time

hosp_clean_time <- hosp_rass_clean[, .(hospitalization_id, begin_imv, end_imv, total_imv_time)]


# Bring in Vitals
vitals <- fread(paste0(data_dir,"vitals.csv.gz"), select = c("hospitalization_id", "recorded_dttm", "vital_category", "vital_value"))[hospitalization_id %in% hosp_id_clean]
setorder(vitals, hospitalization_id, recorded_dttm)

vitals %>% distinct(hospitalization_id)

# Establish Baseline Weight and Height Data
...
labs <- fread(paste0(data_dir,"labs.csv.gz"), select = c("hospitalization_id", "lab_collect_dttm", "lab_order_category", "lab_category", "lab_value", "lab_value_numeric", "reference_unit"))[hospitalization_id %in% hosp_id_clean]
setorder(labs, hospitalization_id, lab_collect_dttm)
setnames(labs, "lab_collect_dttm", "recorded_dttm")


labs %>% distinct(hospitalization_id)

# Pertinent SOFA Tables
...
# P/F Table
#FiO2 Values
resp_supp_fio2 <- respiratory_support[hospitalization_id %in% hosp_id_clean]
...
#Used LOCF to Fill Out fio2 and po2_arterial columns
...
# FINAL MERGER OF THE SOFA TABLE
...
# List of SOFA score columns to calculate max for
sofa_cols <- c("pulm_sofa", "heme_sofa", "neuro_sofa", "gi_sofa", "renal_sofa", "card_sofa")

# Create a new table with the maximum SOFA scores per hospitalization_id
...
# Calculate Average SOFA Score over first 48 hours
...
# Merge Hosp Time Clean with RASS
rass_hosp_clean <- merge(hosp_time_clean, pat_assess_rass, by = "hospitalization_id", all.x = TRUE)
...
# Step 1: Calculate start_time (6 hours before begin_imv)
rass_timeline[, start_time := as.POSIXct(begin_imv) - hours(6)]
...
# Other Covariates Needed for Final Adjustment
```{R}

#BMI


#Admission Source (will probably need to leave out)


#Admission Diagnosis (if surgical would want to adjust for this but won't have in abstract)
...
patient_clean <- patient[patient_id %in% pat_id_clean]
