...
```{r}
packages <- c("zoo", "data.table", "dplyr", "lubridate", "tidyr", "fst", "arrow", "table1", "lme4", "metafor", "yaml", "htmltools")
...
# Generate the summary table by grouping variable
table1(~ age + sex + ethnicity + race + bmi + paralyzed + total_sofa + hospital_id + primary_sedative | language, data = part_two_data)
...
primary_model_adjusted <- lm(percent_deep_sedation ~ language + age + sex + ethnicity + race + bmi + paralyzed + total_sofa + factor(hospital_id) + factor(primary_sedative), data = part_two_data)
